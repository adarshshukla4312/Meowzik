...
import { Play, Pause, SkipForward, SkipBack, Users, Search, Plus, Trash2, Music, Share, X, ListMusic, Shield, ArrowRight } from "lucide-react";
          ...
          {currentTrack ? (
            <div className="flex flex-col items-center animate-in fade-in duration-500">
              ...
              <div className="w-full flex items-center gap-4 mb-8 px-4">
                <span className="text-[12px] text-meowzik-muted font-mono">{formatTime(localProgress)}</span>
                <input 
                  type="range" 
                  min="0" 
          ...
          {showResults && (
            <div className="flex flex-col gap-2 overflow-y-auto pr-2 pb-10">
              {searchResults.slice(0, visibleCount).map((result: any, idx: number) => {
                ...
                return (
                <div key={idx} className="bg-meowzik-surface hover:bg-[#e2e2e2] p-3 rounded-[12px] flex items-center gap-4 transition-colors cursor-pointer group" onClick={() => addToQueue(result)}>
                  <img 
                  ...
                  <div className="w-10 h-10 rounded-full bg-meowzik-bg text-meowzik-accent flex items-center justify-center opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                    <Plus size={20} />
                  </div>
                </div>
                );
